...
namespace Arrowgene.Ddon.GameServer.Handler
{
    public class GachaBuyHandler : GameRequestPacketHandler<C2SGachaBuyReq, S2CGachaBuyRes>
    {
        ...
        public override S2CGachaBuyRes Handle(GameClient client, C2SGachaBuyReq request)
        {
            ...
            {
                Logger.Error($"Gacha ID {request.GachaId} not found in GachaAsset");
                return res;
            }

            var lootBox = gachaAsset.GachaInfoList[request.GachaId];

            // Determine number of draws based on DrawGroupId (defaults to 1)
            uint numDraws = 1;
            if (lootBox.DrawGroupNumDraws.TryGetValue(request.DrawGroupId, out var configuredDraws))
            {
                numDraws = configuredDraws;
            }

            Logger.Info($"Player {client.Character.CharacterId} pulling gacha {request.GachaId} ({lootBox.Name}), DrawGroupId: {request.DrawGroupId}, NumDraws: {numDraws}");
            ...
            switch (request.SettlementId)
            {
                ...
                case 2:
                    ...
                    break;
                default:
                    Logger.Error($"Invalid SettlementId: {request.SettlementId}");
                    return res;
            }

            Server.WalletManager.RemoveFromWalletNtc(client, client.Character, walletType, request.Price);

            // Get the item pool from the first draw group
            if (lootBox.DrawGroups.Count == 0 || lootBox.DrawGroups[0].GachaDrawList.Count == 0)
            {
                Logger.Error($"Gacha {request.GachaId} has no draw configuration");
                return res;
            }

            var itemPool = lootBox.DrawGroups[0].GachaDrawList[0].GachaItemInfo;
            if (itemPool.Count == 0)
            {
                Logger.Error($"Gacha {request.GachaId} has no items in pool");
                return res;
            }

            // Draw the specified number of items
            for (int i = 0; i < numDraws; i++)
            {
                var drawnItem = itemPool[Random.Shared.Next(itemPool.Count)];
                Logger.Info($"Drew item {i + 1}/{numDraws}: ItemId={drawnItem.ItemId}, Num={drawnItem.ItemNum}, Rank={drawnItem.Rank}");
                res.GachaItemList.Add(drawnItem);
            }

            Logger.Info($"Final GachaItemList count: {res.GachaItemList.Count}");
